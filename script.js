...
const promoCodes = {
    "WEEKEND20": { discount: 20, validUntil: "2023-12-31", type: "percentage" },
    "NEWUSER15": { discount: 15, validUntil: "2023-11-30", type: "percentage" },
    "LONGTERM25": { discount: 25, validUntil: "2023-12-31", type: "percentage" },
    "WELCOME10": { discount: 10, validUntil: "2024-01-31", type: "percentage" }
};
...
function displayRecommendedCars() {
    ...
    recommended.forEach(car => {
        ...
        recommendedCar.innerHTML = `
            ...
            <small>${car.rentalCount}x disewa</small>
        `;
...
${currentPromo ? `Kode Promo: ${currentPromo} (Diskon ${currentDiscount}%)` : ''}
...
Apakah kendaraan tersedia untuk tanggal tersebut?`;
    ...
    // Simulate rental count increase (in real app, this would be server-side)
    if (selectedVehicleType === 'car') {
        const selectedCarId = parseInt(carSelect.value);
        const carIndex = carsData.findIndex(car => car.id === selectedCarId);
        if (carIndex !== -1) {
            carsData[carIndex].rentalCount++;
            carsData[carIndex].available--;
            displayRecommendedCars();
        }
    }
...
window.addEventListener('scroll', function() {
    ...
    if (window.scrollY > 100) {
        header.style.backgroundColor = 'rgba(241, 250, 238, 0.98)';
        ...
        header.style.backgroundColor = 'rgba(241, 250, 238, 0.95)';
